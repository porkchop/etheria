...
for (var col = 0; col < (mapsize - 1); col++) {
    for (var row = 0; row < (mapsize - 1); row++) {
      ...
      offers[col][row] = etheria.getOffers(col,row);
      offerers[col][row] = etheria.getOfferers(col,row);
      if(owners[col][row] != 0x0000000000000000000000000000000000000000)
              console.log(col + "," + row + " " + owners[col][row] + " offers=" + offers[col][row] + " offerers=" + offerers[col][row]);
    }
}
*/
